"""Build the whole rendered corpus: N fake sheets + their ground truth.
    ...
    python src/render/build_corpus.py --n 8 --debug          # draw bbox overlays

Writes, per sheet:
    sheets/SHEET00001.png    the rendered, degraded image
    truth/SHEET00001.json    true area, storeys, and every region's box
    labels/SHEET00001.txt    the same regions in YOLO format
    manifest.jsonl           one line per sheet, all the above combined
"""
...
def build(n: int, out: Path, resplan: str | None, seed: int,
    ...
    """Generate n plans, render each as a degraded sheet, and save everything."""
    ...
    for i, plan in enumerate(source[:n]):
        ...
        # Vary the site plan's scale so it's not always the same number.
...
def main() -> None:
    """Parse command-line args and build the corpus."""
    ap = argparse.ArgumentParser()
    ap.add_argument("--n", type=int, default=200)
    ap.add_argument("--out", type=Path, default=ROOT / "data" / "rendered")
